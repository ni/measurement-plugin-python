...
from ni_measurement_plugin_sdk_service._internal.grpc_servicer import _frame_metadata_dict
...
def _get_module_name_and_service_class(
    available_measurement_services: Sequence[ServiceInfo],
) -> Tuple[str, str]:
...
def _get_output_metadata_by_id(
    ...
    for output in metadata.measurement_signature.outputs:
        output_metadata_by_id[output.field_number] = ParameterMetadata.initialize(
            display_name=output.name,
            type=output.type,
            repeated=output.repeated,
            default_value=None,
...
def create_client(
    ...
    configuration_metadata_by_id = _get_configuration_metadata_by_id(metadata, measurement_service_class)
