...
@measurement_service.configuration("number_of_samples", nims.DataType.UInt64, 60000)
...
def measure(pin_name: str, sample_rate: float, number_of_samples: int) -> Tuple[List[float]]:
    ...
    with GrpcChannelPool() as grpc_channel_pool:
        ...
        pin_map_client = PinMapClient(discovery_client= discovery_client,grpc_channel_pool= grpc_channel_pool)
        pin_map_client.update_pin_map(r"D:\Meas-Link\repos\measurementlink-python\examples\nidaqmx_analog_input\NIDAQmxAnalogInput.pinmap")
        ...
        with measurement_service.context.reserve_session(pin_name) as reservation:
            with reservation.create_nidaqmx_task() as session_info:
                ...
                timeout = min(measurement_service.context.time_remaining, 1000.0)
